...
function rowHeader(data, decoder) {
...
function rowFuncs(view, offset, length, dataType, decoder) {
    ...
    switch (dataType) {
        ...
        case 'D': {
            return new Date(Number.parseInt(textData.slice(0, 4), 10), Number.parseInt(textData.slice(4, 6), 10) - 1, Number.parseInt(textData.slice(6, 8), 10));
        }
        case 'L': {
...
function parseRow(view, rowHeaders, decoder) {
    const out = {};
    let offset = 0;
...
export default async function parseDbf(stream, encoding) {
